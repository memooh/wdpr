...
public class KliniekContext : IdentityDbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        {
            entity.HasOne(x => x.Behandelaar).WithMany(x => x.Clienten).HasForeignKey(x => x.BehandelaarId).IsRequired(false).OnDelete(DeleteBehavior.Restrict);
        }
        );
        ...
        builder.Entity<IdentityRole>().HasData(new IdentityRole { Name = "Moderator", NormalizedName = "MODERATOR", Id = Guid.NewGuid().ToString(), ConcurrencyStamp = Guid.NewGuid().ToString() });
